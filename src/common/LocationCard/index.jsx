...
const LocationCard = ({ title, services, industries, description, isHQ = false }) => (
    <div className="p-8 md:p-12 border-b lg:border-r border-[var(--border-subtle)] last:border-r-0 last:border-b-0 hover:bg-[var(--bg-primary)] active:bg-[var(--bg-primary)] transition-colors duration-300">
        <h3 className="font-display text-3xl font-bold mb-4 text-[var(--text-primary)]">
            {title}
            {isHQ && <span className="text-[#2AB182] text-sm align-top ml-2">(HQ)</span>}
        </h3>
        {description && <p className="font-body text-[var(--text-muted)] mb-6">{description}</p>}
        ...
        {industries && (
            <div>
                ...
                <p className="font-body text-sm text-[var(--text-muted)]">{industries}</p>
            </div>
        )}
    </div>
);
