...
const ClientsIndustriesPage = ({ onContactClick }) => {
    return (
        <div className="pt-32 min-h-screen">
            ...
            <section className="py-16 md:py-24 px-6 md:px-12 border-b border-[var(--border-subtle)] bg-[var(--bg-primary)]">
                <div className="max-w-[2000px] mx-auto">
                    <h2 className="font-display text-4xl font-bold uppercase mb-12 text-[var(--text-primary)] text-center">Highlighted Clients</h2>
                    <div className="grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4  gap-6 max-w-7xl mx-auto ">
                        {clientList.map((client, index) => (
                            <div key={index} className="p-4 mt-4 border border-[var(--border-subtle)] flex items-center justify-center text-center bg-[var(--bg-card)]/50 hover:border-[#2AB182] active:border-[#2AB182] transition-colors duration-300">
                                <p className="font-body text-sm font-medium text-[var(--text-primary)]">{client}</p>
                            </div>
                        ))}
                    </div>
                </div>
