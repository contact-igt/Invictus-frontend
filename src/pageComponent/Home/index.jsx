...
const HomePage = ({ activeStep, setActiveStep, stepRefs, activeFaq, toggleFaq }) => {
  ...
  return (
    <div className="pt-20">
      ...
      <section id="hero" className="min-h-[400px] md:min-h-[500px] lg:min-h-[600px] flex flex-col justify-end pb-12 px-6 md:px-12 pt-18 relative border-b border-[var(--border-subtle)]">
        <div className="max-w-[1600px] w-full mx-auto relative z-10">
          <div className="mb-12">
            ...
            <h1 className="font-display font-bold text-[12vw] md:text-[8vw] leading-[0.85] tracking-tight text-[var(--text-primary)] uppercase">
              Marketing<br />
              That Pays<br />
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 w-full border-t border-[var(--border-subtle)] pt-8">
            <p className="font-body max-w-md text-[var(--text-muted)] text-sm md:text-lg leading-relaxed">
              We don’t sell clicks or impressions. We build scalable growth systems powered by ads, data, and AI.
            </p>

            <button
              onClick={() => router.push('/contact')}
              className="group flex items-center gap-4 bg-[#2AB182] text-black px-8 py-4 rounded-none font-bold text-sm uppercase tracking-wider hover:bg-[var(--text-primary)] hover:text-[var(--bg-primary)] active:bg-[var(--text-primary)] active:text-[var(--bg-primary)] transition-colors duration-300 font-body"
            >
              Start Project
      ...
      <div className="border-b border-[var(--border-subtle)] bg-[var(--bg-card)]/50 backdrop-blur-sm py-6 overflow-hidden relative z-10">
        <div className="animate-marquee whitespace-nowrap flex gap-12 items-center">
          {[1, 2, 3, 4].map((i) => (
            <React.Fragment key={i}>
              ...
              <span className="font-display text-4xl md:text-6xl font-bold text-[var(--text-primary)] uppercase">Data Driven</span>
              <div className="w-3 h-3 bg-[#2AB182] rounded-full"></div>
      ...
      <section className="border-b border-[var(--border-subtle)] relative z-10">
        <div className="max-w-[1600px] mx-auto grid md:grid-cols-2">
          <div className="p-8 md:p-12 lg:p-20 border-b md:border-b-0 md:border-r border-[var(--border-subtle)] flex flex-col justify-between min-h-[280px] md:min-h-[400px] backdrop-blur-sm">
            <span className="font-body text-xs text-[var(--text-muted)] uppercase tracking-widest mb-4">(01) — Philosophy</span>
            <h2 className="font-display text-4xl md:text-5xl font-bold uppercase leading-tight text-[var(--text-primary)]">
              Not an Agency.<br />
              A <span className="text-[#2AB182]">Growth Partner.</span>
            </h2>
          </div>
          <div className="p-8 md:p-12 lg:p-20 flex flex-col justify-center bg-[var(--navbar-bg)] backdrop-blur-sm">
            <p className="font-body text-lg md:text-xl text-[var(--text-muted)] leading-relaxed mb-10">
            ...
            <ul className="space-y-6 font-display text-lg md:text-xl">
              {[
                ...
                <li key={index} className="flex items-start lg:items-center gap-4 text-[var(--text-primary)] group">
      ...
      <section className="py-16 md:py-24 border-b border-[var(--border-subtle)] px-6 md:px-12 relative z-10">
        <div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row justify-between items-center text-center gap-8">
          <h2 className="font-display text-4xl md:text-5xl font-bold uppercase text-[var(--text-primary)] leading-tight">
            Our Core <span className="text-[#2AB182]">Capabilities</span>
          </h2>
          <p className="font-body max-w-lg text-[var(--text-muted)]">
            We build scalable growth systems powered by ads, data, and AI. View the breakdown of our 4 core services.
          </p>
          <button
            onClick={() => handleNavClick('/services')}
            className="group flex items-center gap-3 border border-[#2AB182] text-[#2AB182] px-6 py-3 font-body text-sm uppercase tracking-wider hover:bg-[#2AB182] hover:text-[var(--bg-primary)] active:bg-[#2AB182] active:text-[var(--bg-primary)] transition-all duration-300"
          >
      ...
      <section id="industries" className="border-b border-[var(--border-subtle)] relative z-10">
        <div className="max-w-[1600px] mx-auto">
          <div className="p-8 md:p-12 border-b border-[var(--border-subtle)] text-center">
            <h2 className="font-display text-4xl md:text-5xl font-bold uppercase text-[var(--text-primary)] leading-tight">
              Industries We Transform
              <span className="text-[#2AB182]"> (See Our Client Portfolio)</span>
            </h2>
            <p className="font-body text-[var(--text-muted)] mt-4">We go deep into niches where performance truly matters, from Healthcare to E-commerce.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 min-h-[50vh]">
            ...
            {["Healthcare", "EdTech", "E-commerce", "Automobile"].map((name, index) => (
              <div
                key={index}
                className={`p-8 border-b md:border-b-0 border-r border-[var(--border-subtle)] last:border-r-0 flex flex-col justify-center items-center group hover:bg-[#2AB182] hover:text-[var(--bg-primary)] active:bg-[#2AB182] active:text-[var(--bg-primary)] transition-colors duration-500 bg-[var(--navbar-bg)] backdrop-blur-sm`}
              >
                <h3 className="font-display text-2xl font-bold uppercase mb-4">{name}</h3>
              </div>
            ))}
          </div>
          <div className="p-8 text-center border-t border-[var(--border-subtle)]">
            <button
              onClick={() => handleNavClick('/clients')}
              className="group flex items-center justify-center mx-auto gap-3 bg-[#2AB182] text-black px-6 py-3 font-body text-sm uppercase tracking-wider hover:bg-[var(--text-primary)] hover:text-[var(--bg-primary)] active:bg-[var(--text-primary)] active:text-[var(--bg-primary)] transition-all duration-300"
            >
      ...
      {/* Stats Section */}
      <section className="border-b border-[var(--border-subtle)] py-16 md:py-24 px-6 md:px-12 bg-[var(--bg-card)]/80 backdrop-blur-sm relative z-10">
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row items-center justify-center gap-12 md:gap-30">
          <div>
            ...
            <p className="font-body text-[var(--text-muted)] uppercase tracking-widest text-sm">Avg. Client ROAS</p>
          </div>
          <div>
            <h2 className="font-display text-6xl md:text-7xl lg:text-9xl font-bold text-[var(--text-primary)] mb-2">
              <CountUp end={40} duration={2} prefix="-" suffix="%" enableScrollSpy={true}
                scrollSpyOnce={true} />
            </h2>
            <p className="font-body text-[var(--text-muted)] uppercase tracking-widest text-sm">Cost Per Lead Reduction</p>
          </div>
        </div>
      </section>

      {/* SECTION 5 — Process Steps (Methodology) */}
      <section id="process" className="py-16 md:py-24 border-b border-[var(--border-subtle)] relative z-10 bg-[var(--bg-primary)]">
        <div className="max-w-[1600px] mx-auto px-6 md:px-12">
          <div className="text-center mb-20">
            ...
            <h2 className="font-display text-4xl md:text-6xl font-bold text-[var(--text-primary)] leading-tight max-w-4xl mx-auto">
              The 4-Step Performance Engine
            </h2>
            <p className="font-body text-[var(--text-muted)] mt-4">Performance is engineered — not guessed.</p>
          </div>

          <div className="relative">
            {/* Horizontal timeline line (Desktop) */}
            <div className="hidden lg:block absolute top-[24px] left-0 w-full h-[1px] bg-[var(--border-subtle)] z-0"></div>
            {/* Vertical timeline line (Mobile) */}
            <div className="lg:hidden absolute top-0 left-[22px] w-[1px] h-full bg-[var(--border-subtle)] z-0">
              {/* Mobile Progress Bar */}
              <div
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`
                        ...
                        ${isMobileActive ? 'border-[#2AB182] lg:border-[var(--border-subtle)] lg:group-hover:border-[#2AB182]' : 'border-[var(--border-subtle)] lg:group-hover:border-[#2AB182]'}
                        bg-[var(--bg-primary)]
                      `}>
                      <div className={`
                            w-4 h-4 rounded-full transition-all duration-500
                            ${isMobileActive ? 'bg-[#2AB182] lg:bg-[var(--text-secondary)] lg:group-hover:bg-[#2AB182]' : 'bg-[var(--text-secondary)] lg:group-hover:bg-[#2AB182]'}
                      `}></div>
                    </div>
                    ...
                    <h3 className={`
                        ...
                        ${isMobileActive ? 'text-[#2AB182] lg:text-[var(--text-primary)] lg:group-hover:text-[#2AB182]' : 'text-[var(--text-primary)] lg:group-hover:text-[#2AB182]'}
                      `}>
                    ...
                    <ul className="space-y-3 font-body text-sm transition-colors duration-500">
                      {step.items.map((item, idx) => (
                        <li key={idx} className={`
                            ...
                            ${isMobileActive ? 'text-[var(--text-primary)] lg:text-[var(--text-muted)] lg:group-hover:text-[var(--text-primary)]' : 'text-[var(--text-muted)] lg:group-hover:text-[var(--text-primary)]'}
                          `}>
                          <div className={`
                              w-1 h-1 rounded-full flex-shrink-0 mt-2 transition-colors duration-500
                              ${isMobileActive ? 'bg-[#2AB182] lg:bg-[var(--text-muted)] lg:group-hover:bg-[#2AB182]' : 'bg-[var(--text-muted)] lg:group-hover:bg-[#2AB182]'}
                          `}></div>
                          <span>{item}</span>
                        </li>
                      ))}
      ...
      <section className="py-16 md:py-24 border-b border-[var(--border-subtle)] px-6 md:px-12 relative z-10">
        <div className="max-w-3xl mx-auto">
          <h2 className="font-display text-4xl font-bold uppercase mb-14 text-[var(--text-primary)] text-center">FAQ</h2>
          <div className="space-y-6">
            {[
              ...
              <div key={index} className="group cursor-pointer" onClick={() => toggleFaq(index)}>
                <div className={`flex justify-between items-center border-b border-[var(--border-subtle)] pb-4 transition-colors ${activeFaq === index ? 'border-[#2AB182]' : ''}`}>
                  <h3 className={`font-body text-lg md:text-xl font-medium transition-all ${activeFaq === index ? 'text-[#2AB182] pl-4' : 'text-[var(--text-primary)] group-hover:text-[#2AB182]'}`}>
                    {item.q}
                  </h3>
                  {activeFaq === index ?
                    <Minus className="text-[#2AB182]" size={24} /> :
                    <Plus className="text-[var(--text-muted)] group-hover:text-[var(--text-primary)]" size={24} />
                  }
                </div>
      ...
      <section id="contact-cta" className="py-16 md:py-32 border-t border-[var(--border-subtle)] text-center px-6 bg-[var(--bg-card)]/30">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="font-body text-[var(--text-muted)] text-xl mb-12">
            ...
            <span className="text-[var(--text-primary)]">If you’re growth-ready and want a partner who owns the numbers with you:</span>
          </p>
          <button
            onClick={() => router.push('/contact')}
