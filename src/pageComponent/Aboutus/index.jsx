...
import { Target, Zap, BarChart3, Users, CheckCircle } from "lucide-react";

const AboutPageComponent = ({ onContactClick }) => {
  const values = [
    {
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 ">
            {values.map((val, idx) => (
              <div
                key={idx}
                className="p-8 border border-[#333] bg-[#0E0E0E] hover:border-[#2AB182] transition-colors duration-300 group"
              >
                <div className="mb-6 p-4 bg-[#1A1A1A] rounded-full w-fit group-hover:bg-[#2AB182]/10 transition-colors">
                  {val.icon}
                </div>
        ...
        <div className="max-w-[1600px] mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="font-body text-[#888]">
              We’re a performance marketing agency built on three pillars:
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {pillars.map((pillar, idx) => (
              <div
                key={idx}
                className="p-8 border border-[#333] bg-[#0E0E0E] hover:border-[#2AB182] transition-colors duration-500 group"
              >
                <h3 className="font-display text-2xl text-[#2AB182] font-bold mb-6">
                  {pillar.title}
                </h3>
                <p className="font-body text-[#BBB] leading-relaxed mb-4">
                  {pillar.desc}
                </p>
                {pillar.extra && (
      ...
      <section className="py-24 px-6 md:px-12 border-t border-[#333] bg-[#1A1A1A]/30">
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              "We don't sell content. We sell growth.",
              "We don't do guesswork. We do data.",
              "We don't chase likes. We chase revenue.",
              "We don't act like vendors. We think like partners.",
              "We don't work with everyone. Serious brands only.",
            ].map((item, idx) => (
              <div
                key={idx}
                className="flex items-center gap-4 p-6 border border-[#333] bg-[#0E0E0E]"
              >
                <div className="w-2 h-2 bg-[#2AB182] rounded-full shrink-0"></div>
                <span className="font-display text-white font-bold uppercase text-lg">
                  {item}
                </span>
              </div>
            ))}
            <div className="flex items-center justify-center p-6 border border-[#2AB182] bg-[#2AB182]/10">
              <span className="font-body text-[#2AB182] font-bold text-center">
                If you need real results, Invictus is built for you.
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <p className="font-body text-[#888] text-xl mb-4">
            ...
            <span className="text-white">
              But we’re the team that owns performance with you.
            </span>
          </p>
          <button
